...
class Archives extends Model
{
    ...
    public function getIscommentAttr($value, $data)
    {
        ...
        if ($iscomment) {
            $iscomment = $value ? $value : self::$config['iscomment'];
        }
        return $iscomment;
    }
    ...
    public function getTaglistAttr($value, &$data)
    {
        ...
        $time = $data['releasetime'] ?? time();
        $list = [];
        foreach ($tags as $k => $v) {
            $vars = [':name' => $v, ':diyname' => $v, ':id' => isset($tagList[$v]) ? $tagList[$v] : '0', ':year' => date("Y", $time), ':month' => date("m", $time), ':day' => date("d", $time)];
    ...
    private function buildUrl($value, $data, $domain = false)
    {
        ...
        return addon_url('cms/archives/index', $vars, $suffix, true);
    }
